...
function genesis_seo_site_title() {
	...
	$wrap = is_home() && genesis_get_seo_option('home_h1_on') ? $wrap : 'h1';
...
function genesis_robots_meta() {
	...
	if ( is_front_page() ) {
		...
		$meta['nofollow'] = genesis_get_seo_option('home_nofollow') ? 'noindex' : $meta['noindex'];
	...
	if ( is_singular() ) {
		...
		$meta['nofollow'] = genesis_get_custom_field('_genesis_nofollow') ? 'nofollow' : $meta['noindex'];
